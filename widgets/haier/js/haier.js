...
// this code can be placed directly in haier.html
vis.binds.haier = {
    version: "0.5.0",
    showVersion: function () {
        if (vis.binds.haier.version) {
            console.log('Version haier: ' + vis.binds.haier.version);
            vis.binds.haier.version = null;
        }
    },
	createWidget: function (widgetID, view, data, style) {
        ...
        if (!$div.length) {
            return setTimeout(function () {
                vis.binds.haier.createWidget(widgetID, view, data, style);
            }, 100);
        }
        ...
        if (data.oid) {
            vis.states.bind(data.oid + '.val', function (e, newVal, oldVal) {
                $div.find('.haier-value').html(newVal);
            });
        }
    }
};
	
vis.binds.haier.showVersion();
